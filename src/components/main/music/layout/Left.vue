    ...
    <div class="container-left">
        ...
        <div class="plat-list">
           <el-menu class="plat-menu" style="border-right: none;" :default-active="$router.currentRoute.value.params.id">
                <el-menu-item class="plat" :class="{'is-active':currPlatId==item.id}" @click="changePlat(item)" v-for="(item) in musicStore.menus!.meta.platform" :index="item.id">{{item.name}}</el-menu-item>
            </el-menu>
        </div>
    </div>
</template>
...
const currPlatId = ref($router.currentRoute.value.params.id);
...
const changePlat = async (item:any) => {
    ...
    currPlatId.value = item.id;
    musicEnevt.initDefaultClass();
}

const changeTitle = () => {
    const id = currPlatId.value;
