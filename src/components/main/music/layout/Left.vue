    ...
    <div class="container-left">
        ...
        <div class="plat-list">
           <el-menu class="plat-menu" style="border-right: none;" :default-active="$router.currentRoute.value.params.id">
                <el-menu-item class="plat" :class="{'is-active':musicStore.currentPlat.id==item.id}" @click="changePlat(item)" v-for="(item) in musicStore.menus!.meta.platform" :index="item.id">{{item.name}}</el-menu-item>
            </el-menu>
        </div>
    </div>
</template>
...
const changeTitle = () => {
    ...
    changePlat(child![index].meta.platform?.find(item => item.id == musicStore.currentPlat.id));
}
